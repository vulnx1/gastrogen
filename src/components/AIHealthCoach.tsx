...
export function AIHealthCoach({ healthData, userName, wearableData }: AIHealthCoachProps) {
  ...
  // Use env var for WS URL with sensible default
  const WS_URL = (import.meta as any).env?.VITE_WS_URL ?? "ws://localhost:8000/ws/chat/";
  ...
  // Robust time formatter for Date or string timestamps
  ...
  const handleSendMessage = () => {
    ...
    const userInputSnapshot = inputValue; // snapshot before clearing
    setInputValue('');

    // If socket isn't ready, fall back to local response
    if (!socket || socket.readyState !== WebSocket.OPEN) {
      const aiResponse: Message = {
        id: (Date.now() + 1).toString(),
        content: generateAIResponse(userInputSnapshot),
        sender: 'ai',
        timestamp: new Date(),
        type: 'text'
      };
    ...
    setIsTyping(true);
    try {
      socket.send(JSON.stringify({ message: userInputSnapshot }));
    } catch (e) {
      // Fallback to local response on send failure
      const aiResponse: Message = {
        id: (Date.now() + 1).toString(),
        content: generateAIResponse(userInputSnapshot),
        sender: 'ai',
