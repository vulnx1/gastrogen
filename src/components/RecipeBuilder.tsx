...
export function RecipeBuilder({ userName, onSaveRecipe }: RecipeBuilderProps) {
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      ...
      // Frontend recipe object (for UI/state)
      const localRecipe: Recipe = {
        ...
        cost: Math.round(ingredients.length * 2.5),
        ...
        nutrition: {
          ...
          sugar: Math.round(totalCalories * 0.1 / 4),
      ...
      if (!res.ok) {
        ...
        console.error('Failed to save recipe:', text);
        // Optional: surface to UI via toast
      } else {
        const saved = await res.json();
        // If backend returns id, sync it to local object
        if (saved && saved.id) {
          localRecipe.id = String(saved.id);
        }
      }

      onSaveRecipe(localRecipe);
    } catch (e) {
      console.error(e);
    } finally {
      setSaving(false);
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                setPastRecipes(items);
              } catch (e) {
                console.error('Failed to fetch past recipes', e);
              } finally {
                setLoadingPast(false);
                document.getElementById('past-recipes-section')?.scrollIntoView({ behavior: 'smooth' });
              }
            }}
          >
            {loadingPast ? 'Loading...' : 'Past Recipes'}
          </Button>
        </div>
          ...
          >
            ...
            <div className="flex space-x-4">
              <Button
                ...
                disabled={saving || !recipeName.trim() || ingredients.length === 0 || instructions.length === 0}
